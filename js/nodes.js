var regFill = "pink";
var highlightFill = "darkslategrey";
...
function Tree() {
  ...
  this.removeNode = function (index) {
    ...
    this.text = this.text.filter((e, i) => {
      return i !== index;
    });
      ...
      if (i === 0) {
        ...
        treeContainer
          ...
          .call(
            createLineAttr,
            "black",
            this.data[parent(i)].cx,
            this.data[parent(i)].cy,
            node.cx,
            node.cy
          );
      }
      ...
      if (arr.slice(0, mid).length) {
        treeContainer
          ...
          .call(
            ...
            "black",
            cx,
            radius + depth * ySpacing,
            cx - xSpacing / nextDepth,
            radius + nextDepth * ySpacing
          );
      }
...
function createArray(arr, x, y, width, height) {
  ...
  var elementsArr = arrayContainer
    .selectAll("rect")
    .data(arrayData)
    .enter()
    .append("rect")
...
function createContainer(id, arr, width, height) {
  ...
  let container = d3
    .select(`div#${id}`)
    .append("svg")
    .attr("width", width || box.width * 600 * (0.8 / depth) * 0.75)
